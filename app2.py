...
# --- NEW: Import for local LLM integration ---
from transformers import pipeline

@st.cache_resource
def load_phi2_model():
    generator = pipeline("text-generation", model="microsoft/phi-2", max_length=300)
    return generator

phi2_generator = load_phi2_model()

# ---- App Configuration ----
st.set_page_config(page_title="AI Learning Path Generator", page_icon="🎓", layout="wide")
st.title("🎓 AI-Powered Personalized Learning Path Generator (Now with Phi-2 LLM)")
...
def improved_prompt(subject, marks, semester):
    return (
        ...
        "- Motivational advice\n"
        "Reply with bullet points under '**Week 1**' and '**Week 2**', do not repeat the question or add any headers."
    )

def clean_llm_output(ai_plan):
    # Remove prompt echo, extraneous lines before 'Week 1'
    plan = re.split(r"\*\*Week 1\*\*", ai_plan, maxsplit=1)
    if len(plan) == 2:
        ai_plan = "**Week 1**" + plan[1]
    ai_plan = re.sub(r"(assistant[ :]*|AI mentor[ :]*)", "", ai_plan, flags=re.I)
    # Each bullet point as markdown
...
def generate_llm_plan(weak_subjects, entered_marks, semester):
    ...
    for subject in weak_subjects:
        ...
        with st.spinner(f"AI is generating your personalized plan for {subject}..."):
            ai_raw = phi2_generator(prompt)[0]['generated_text']
        ai_plan = clean_llm_output(ai_raw)
        plan += f"\n\n#### {subject.replace('_',' ')}\n\n{ai_plan}\n\n---\n"
    plan += "*This plan was generated by Microsoft Phi-2 LLM running locally.*"
    return plan
...
st.caption("Powered locally by Microsoft Phi-2 and Tensorflow. No cloud usage. All logic runs on your device.")